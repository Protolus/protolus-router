...
var ProtolusRouter = prime({ //the default engine
    //EnhancedEmitter
    inherits: Emitter,
    ...
    addRoute : function(test, callback){
        ...
        if(type(group) != 'array') group = [group];
    ...
    route : function(url){
        ...
        this.fileLoadComplete(fn.bind(function(){
            ...
            array.forEach(this.routes, function(route){
                if(route.group != '*' && !array.contains(route.group, group)) return;
                if(found) return;
                found = route.test(url);
                if(found === false) found = [];
                else if(found.length > 1) found.shift(); //remove the full selection if we have subselections
                delete found.index;
                delete found.input;
                if(found){
